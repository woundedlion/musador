...
#include <sys/stat.h>
  ...
  // in the win32 case we might get
  // along with including windows.h
...
static LPSTR toANSI(const char *file)
{
  int size = (int)strlen(file) + 1;
  LPWSTR lpWideCharStr = new WCHAR[size];
  if(lpWideCharStr) {
    if(MultiByteToWideChar(CP_UTF8, 0, file, -1, lpWideCharStr, size) > 0) {
      LPSTR lpAnsiCharStr = new CHAR[size];
      if(lpAnsiCharStr) {
        if(WideCharToMultiByte(CP_ACP, 0, lpWideCharStr, -1, lpAnsiCharStr, size, NULL, NULL) > 0) {
          delete [] lpWideCharStr;
          return lpAnsiCharStr;
        }
...
static LPWSTR toUTF16(const char *file)
{
  int size = (int)strlen(file) + 5;
  LPWSTR lpWideCharStr = new WCHAR[size];
  if (lpWideCharStr) {
    wcscpy(lpWideCharStr, L"\\\\?\\");
    if (MultiByteToWideChar(CP_UTF8, 0, file, -1, lpWideCharStr + 4, size) > 0) {
...
File::File(const char *file)
{
  ...
  if(GetVersion() < 0x80000000) {
    LPWSTR nfile = toUTF16(file);
    if(nfile) {
      d->file = _wfopen(nfile, L"r+b");
      delete [] nfile;
    }
  }
  else {
    LPSTR nfile = toANSI(file);
    if(nfile) {
      d->file = fopen(nfile, "r+b");
      delete [] nfile;
    }
  }
#else /* UNIX */
...
ByteVector File::readBlock(ulong length)
{
  ...
  const int count = (int)fread(v.data(), sizeof(char), length, d->file);
  v.resize(count);
  return v;
}
...
void File::insert(const ByteVector &data, ulong start, ulong replace)
{
  ...
  seek(readPosition);
  int bytesRead = (int)fread(aboutToOverwrite.data(), sizeof(char), bufferLength, d->file);
  ...
  while(!buffer.isEmpty()) {
    ...
    seek(readPosition);
    bytesRead = (int)fread(aboutToOverwrite.data(), sizeof(char), bufferLength, d->file);
